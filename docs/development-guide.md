...
### 1.6 SQLiteデータベースとログファイル

アプリケーションは `memory.db` というローカル SQLite データベースを使用します。ファイルが存在しない場合は、サーバー起動時に自動的に作成されます。ログファイル（`*.log`）と共にリポジトリには含めないよう `.gitignore` で除外しているため、環境を再構築する際は特別な手順は不要です。
