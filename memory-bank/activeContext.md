...
現在、types ディレクトリのリファクタリングを継続しています。このプロジェクトは以下のフェーズ（T-0〜T-6）のうち、T-0〜T-4が完了し、次はT-5（共通型の整理）を実施予定です：

- **T-5** (計画中): ルートに残った汎用型を common/ に集約し、使われていない旧ファイルは _legacy-*.ts にリネーム。ESLint で旧ファイル直接参照を禁止。
- **T-6**: バレルをサブフォルダー再エクスポートのみに変更し、全コードの参照を修正（任意）。

### 次のステップ

1. types ディレクトリリファクタリングの T-5 フェーズを開始（共通型の整理）
2. ユーザー認証関連のフックの整理
...
また、types ディレクトリリファクタリングプロジェクトの T-0〜T-4 フェーズが完了しました。これにより：
